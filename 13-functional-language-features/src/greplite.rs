...
impl SearchConfig {
    ...
    /// An improved version of "from_args" that does not call String.clone()
    /// and is therefore more efficient
    /// from_args_iter takes ownership of an iterator instance so that it can
    /// also take ownership of the data in the iterator
    pub fn from_args_iter(
...
pub fn search<'a>(pattern: &str, content: &'a str, case_sensitive: bool) -> Vec<&'a str> {
    ...
    return content
        ...
        .filter(|l| {
            l.contains(&pattern) 
            || (!case_sensitive && l.to_lowercase().contains(&pattern))
        })
        .collect();
}
